const Upgrade = {};
Upgrade.fromStats = function(dmg_lvl, hp_lvl, base_dmg, base_hp) {
  return function(lvl) {
...
const Game = {
  ...
  init: function() {
    ...
    this.player = player;
    player.botTeam[0].backcolor = Game.player_hl;
  ...
  _generateMap: function() {
    ...
    this.map[weaponCell[0]+","+weaponCell[1]] = [make_grenade()];
...
const make_grenade = function() {
  return {
    ...
    cooldown : 10,
    cd_timer : 0,
    describe : function() { return "Grenade"; }
  }
}

const Bot = function(x, y, name, symbol, color, scrap, wild, lvl, upgrade) {
  this.abilities = [];
  this.x = x;
  this.y = y;
  this.tile = symbol;
...
const player = {
  scrap : 0,
  catchers : 0,
  botTeam : [new Bot(-1, -1, "Wolfercycle", 'w', "white", 10, false, 3,
                     Upgrade.wolfercycle)],
  abilities : [],
  name : "You",
  tile : "@",
  color : "#ff0",
  captureMode : false,
  uiMode : "play", // "play", "examine", "scrap"
  examineX : -1,
  examineY : -1,
  describe : () => "", // shortcut notation for function that always returns ""
  keyMap : {
    75 : 0, // k
    85 : 1, // u
    76 : 2, // l
...
player.loseBot = function(deadBot) {
...
player.targetMode = function(code, dir) {
